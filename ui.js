...
function renderMine(){
  ...
  document.getElementById('planet-btn')?.classList.add('planet-touch');
...
function renderTechHTML(){
  ...
  const items=TECH[activeTechCat]||[];
  const researched=items.filter(t=>hasTech(t.id)).length;
  let h=`<div class="hero-panel">
    <div class="hero-top">
      <div>
        <div class="hero-kicker">лаборатория и технологии</div>
        <div class="hero-title">Технологическое древо</div>
        <div class="hero-sub">Открывай улучшения по веткам. Сначала видно, что уже исследовано, что доступно сейчас и что ещё требует предыдущую технологию.</div>
      </div>
      <div class="hero-icon">🔬</div>
    </div>
    <div class="mini-grid">
      <div class="mini-stat"><div class="l">Наука</div><div class="v">${fmt(Math.floor(G.rp))}</div></div>
      <div class="mini-stat"><div class="l">Кредиты</div><div class="v">${fmt(G.cr)}</div></div>
      <div class="mini-stat"><div class="l">Ветка</div><div class="v">${cats[activeTechCat].split(' ').slice(1).join(' ')}</div></div>
      <div class="mini-stat"><div class="l">Изучено</div><div class="v">${researched}/${items.length}</div></div>
    </div>
  </div>
  <div class="tech-cats">`;
  ...
  items.forEach(t=>{
    ...
    const status=done?`<div class="td">✅ Исследовано</div>`:!reqOk?`<div class="tl">🔒 Нужна предыдущая технология</div>`:`<div class="tc">💰${fmt(t.cost)} · 🔬${t.rp} НО</div>`;
...
function renderRangersHTML(){
  ...
  const nextRank=RANKS.filter(r=>G.killCount<r.minKills||G.lvl<r.minLvl).slice(0,1)[0];
  const rangerClass=G.rangerClass||'free';
  const classNames={free:'Свободный рейнджер',trader:'Торговец',hunter:'Охотник',scientist:'Исследователь',commander:'Командор'};
  const classIcons={free:'🛰️',trader:'💰',hunter:'⚔️',scientist:'🧭',commander:'👑'};
  let h=`<div class="ranger-overview">
    <div class="ranger-head">
      <div>
        <div class="title">класс рейнджера</div>
        <div class="name">${classIcons[rangerClass]||'🛰️'} ${classNames[rangerClass]||'Свободный рейнджер'}</div>
        <div class="sub">${rangerClass==='free'?'Сбалансированный путь без перекоса. Хорош для плавного старта и адаптации к миру.':rangerClass==='trader'?'Делает ставку на кредиты, маршруты и логистику.':rangerClass==='hunter'?'Сильнее в охоте, зачистке угроз и боевых лицензиях.':rangerClass==='scientist'?'Лучше раскрывает исследования, технологии и редкие открытия.':'Давит влиянием, поддержкой и контролем систем.'}</div>
      </div>
      <div class="ranger-lvl"><div class="value">Ур. ${G.classLevel||1}</div><div class="label">xp ${Math.floor(G.classXp||0)}</div></div>
    </div>
    <div class="ranger-stats">
      <div class="ranger-stat"><div class="n">${G.power||0}</div><div class="l">Сила</div></div>
      <div class="ranger-stat"><div class="n">${G.honor||0}</div><div class="l">Честь</div></div>
      <div class="ranger-stat"><div class="n">${(G.influence?.[G.galaxyId||'alpha'])||0}</div><div class="l">Влияние в галактике</div></div>
      <div class="ranger-stat"><div class="n">${G.combatLicense||1}</div><div class="l">Боевая лицензия</div></div>
    </div>
    <div class="path-grid">
      ${[['free','Свободный рейнджер'],['trader','Торговец'],['hunter','Охотник'],['scientist','Исследователь'],['commander','Командор']].map(([id,name])=>`<button class="path-btn${rangerClass===id?' active':''}" onclick="setRangerClass('${id}')">${classIcons[id]||'•'} ${name}</button>`).join('')}
    </div>
  </div>`;

  h+=`<div class="hero-panel" style="margin-bottom:12px">
    <div class="hero-top">
      <div>
        <div class="hero-kicker">звание и статус</div>
        <div class="hero-title">${rankIcons[rank.name]||'🌟'} ${rank.name}</div>
        <div class="hero-sub">Следи за званием, силой и репутацией. Эти параметры открывают более сложные угрозы и более дорогую технику.</div>
      </div>
      <div class="hero-icon">🪪</div>
    </div>
    <div class="banner-line">
      <span class="pill">⚔️ Убийств: ${G.killCount}</span>
      <span class="pill">🧬 Уровень: ${G.lvl}</span>
      <span class="pill">👽 Пришельцев: ${G.alienKills||0}</span>
      ${nextRank?`<span class="pill">Следующее: ${nextRank.name}</span>`:''}
    </div>
  </div>`;

  h+=`<div class="finance-card">
    <div>
      <div class="title">Финансовый статус</div>
      <div class="meta">Долг: <b style="color:var(--gold)">${fmt(G.debt||0)} кр.</b> · Лимит: ${fmt(G.debtLimit||0)}<br>Разорения: ${G.bankruptcies||0} · Провалено контрактов: ${G.failedContracts||0}</div>
    </div>
    <button class="btn btn-sm btn-g" onclick="repayDebt()" ${(G.debt||0)>0 && G.cr>0 ? '' : 'disabled'}>Погасить долг</button>
  </div>`;

  h+=`<div class="sh">Лицензии на угрозы</div>`;
  const licenses=[
    {lvl:1,name:'Лицензия патруля',ok:(G.power||0)>=0&&(G.honor||0)>=-50&&(G.completedContracts||0)>=0,req:'Сила 0 · Честь -50 · Контракты 0'},
    {lvl:2,name:'Лицензия рейдера',ok:(G.power||0)>=80&&(G.honor||0)>=0&&(G.completedContracts||0)>=2,req:'Сила 80 · Честь 0 · Контракты 2'},
    {lvl:3,name:'Лицензия ударной зоны',ok:(G.power||0)>=180&&(G.honor||0)>=10&&(G.completedContracts||0)>=6,req:'Сила 180 · Честь 10 · Контракты 6'}
  ];
  licenses.forEach(l=>{
    h+=`<div class="license-card${l.ok?'':' locked'}"><div class="license-row"><div><div class="license-name">${l.ok?'✅':'🔒'} ${l.name}</div><div class="license-req">${l.req}</div></div><div class="pill">Ур. ${l.lvl}</div></div></div>`;
  });
  ...
  RANGER_BOTS.forEach(r=>{
    ...
    const ok=G.cr>=cost;
    h+=`<div class="ranger-bot-card">
      <div class="rg-icon">${r.icon}</div>
      <div class="rg-info">
        <div class="rg-name">${r.name}</div>
        <div class="rg-action">${r.desc}</div>
        <button class="btn btn-sm${ok?' btn-p':''}" style="margin-top:8px" ${ok?'':'disabled'} onclick="buyRanger('${r.id}')">Нанять · 💰${fmt(cost)}</button>
      </div>
      <div class="rg-owned">${owned}</div>
    </div>`;
  });
...
function setRangerClass(id){
  G.rangerClass=id;
  toast(`🛰️ Путь: ${id==='free'?'Свободный рейнджер':id==='trader'?'Торговец':id==='hunter'?'Охотник':id==='scientist'?'Исследователь':'Командор'}`,'good');
  haptic('medium');
  renderMoreTab();
}
function repayDebt(){
  const debt=Math.max(0,Math.floor(G.debt||0));
  if(!debt){toast('Долгов нет','warn');return;}
  const pay=Math.min(Math.floor(G.cr||0),debt);
  if(pay<=0){toast('💸 Недостаточно кредитов','bad');return;}
  G.cr-=pay; G.debt=Math.max(0,debt-pay);
  toast(`💳 Погашено ${fmt(pay)} кр`,'good');
  renderMoreTab(); updateHUD();
}

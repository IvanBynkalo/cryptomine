...
function updateHUD(){
  ...
  const pidx=(G.currentPlanetBySystem&&Number.isInteger(G.currentPlanetBySystem[G.sys]))?G.currentPlanetBySystem[G.sys]:0;
  const locName=((sysObj?.planets&&sysObj.planets[pidx])||sysObj?.emoji||'')+' '+sysName;
  ...
  ['g-loc','t-loc','q-loc'].forEach(id=>{const el=document.getElementById(id);if(el) el.textContent=locName.trim();});
...
function renderMine(){
  ...
  const pidx=(G.currentPlanetBySystem&&Number.isInteger(G.currentPlanetBySystem[G.sys]))?G.currentPlanetBySystem[G.sys]:0;
  document.getElementById('planet-em').textContent=(sys.planets&&sys.planets[pidx])||sys.emoji;

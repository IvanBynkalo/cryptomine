...
const MARKET_ICON_BY_CATEGORY={
  basic:'📦', industrial:'⚙️', military:'🛡️', science:'🔬', regional:'🪐', unique:'✨', luxury:'💎', contract:'📜'
};
function getMarketCatalog(){
  return Array.isArray(globalThis.MARKET_CATALOG) ? globalThis.MARKET_CATALOG : [];
}
function getMarketGoodDef(gId){
  if(globalThis.GOODS && GOODS[gId]) return GOODS[gId];
  const it=globalThis.MARKET_BY_ID?.[gId] || getMarketCatalog().find(x=>x.id===gId);
  if(!it) return {id:gId,name:gId,icon:'📦',base:0,category:'misc',volume:1};
  return {
    id:it.id,
    name:it.name,
    icon:it.icon || MARKET_ICON_BY_CATEGORY[it.category] || '📦',
    base:it.basePrice || it.base || 0,
    category:it.category,
    volume:it.volume || 1,
    rarity:it.rarity || 'common',
    licenseRequired:it.licenseRequired || null,
    unique:!!it.unique,
    description:it.description || ''
  };
}
function _sysHash(str){
  let h=0; for(let i=0;i<str.length;i++) h=((h<<5)-h)+str.charCodeAt(i); return Math.abs(h);
}
function getSystemMarketGoods(sys){
  ...
  const takeMap={basic:8,industrial:6,military:6,science:6,regional:6,unique:3,luxury:4,contract:5};
  const out=[];
  const add=id=>{ if(id && !out.includes(id)) out.push(id); };
  (sys.goods||[]).forEach(add);
  const cats=catMap[sys.type] || ['basic','industrial','regional'];
  const all=getMarketCatalog();
  const seed=_sysHash(sys.id);
  cats.forEach((cat, idx)=>{
    const arr=all.filter(x=>x.category===cat);
    if(!arr.length) return;
    const count=Math.min(takeMap[cat]||4, arr.length);
...
function initPrices(){
  SYSTEMS.forEach(sys=>{
    ...
    getSystemMarketGoods(sys).forEach(gId=>{
...
function fluctuatePrices(bigEvent=false){
  SYSTEMS.forEach(sys=>{
    ...
    if(!G.priceHistory[sys.id]) G.priceHistory[sys.id]={};
    getSystemMarketGoods(sys).forEach(gId=>{
      const def=getMarketGoodDef(gId); const base=def.base||0, cur=G.prices[sys.id][gId]||base;
  ...
  if(numTraders>0){
    for(let i=0;i<numTraders*2;i++){
      ...
      const def=getMarketGoodDef(good); const base=def.base||0, cur=G.prices[sys.id][good]||base;
...
function refreshQuests(){
  ...
  MAYORS.forEach(m=>{
    ...
    if(type==='deliver'){
      ...
      q.title=`Доставить ${getMarketGoodDef(good).name} → ${dest.name}`;
      q.desc=`Возьмите ${amt}× ${getMarketGoodDef(good).name} и доставьте в систему ${dest.name}. Груз можно сдать прямо в контрактном центре.`;
